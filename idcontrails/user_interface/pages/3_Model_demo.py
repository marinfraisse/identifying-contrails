...
if 'choice_of_contrail' not in st.session_state:
    st.session_state['choice_of_contrail'] = "Nothing"
...
############################### SELECTBOX VERSION ###############################
            # Retrieving only the 3 bands we want and the true mask
            st.markdown("***")
            list_test_contrails = os.listdir(os.path.join(absolute_path_root, "dataset_sample_app"))
            contrail_choice = st.selectbox("Now let's choose and build an image", list_test_contrails)
            st.session_state['choice_of_contrail'] = contrail_choice
            if st.session_state['choice_of_contrail'] != "Nothing":
                st.success("✅ Files loaded")
                band11 = np.load(os.path.join(absolute_path_root, "dataset_sample_app", contrail_choice,"band_11.npy"))
                band14 = np.load(os.path.join(absolute_path_root, "dataset_sample_app", contrail_choice,"band_14.npy"))
                band15 = np.load(os.path.join(absolute_path_root, "dataset_sample_app", contrail_choice,"band_15.npy"))
                y_true = np.load(os.path.join(absolute_path_root, "dataset_sample_app", contrail_choice,"human_pixel_masks.npy"))
